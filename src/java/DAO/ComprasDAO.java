...
public class ComprasDAO {
    ...
    public int IdCompra() {
        int idc = 0;
        String sql = "select max(idComprasWeb) from comprasweb";
        try {
    ...
    public int guardarCompra(CompraOnline co) {
        String sql = "insert into comprasweb(idCliente,idPago, FechaCompras,Monto,Estado)values(?,?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public int guardarDetalleCompra(DetalleCompraOnline dc) {
        String sql = "INSERT INTO detalle_comprasweb(idProducto,idComprasweb, Cantidad, PrecioCompra)VALUES(?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List misCompras(int id) {
        ...
        String sql = "select * from comprasweb where idCliente=" + id;
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                CompraOnline com = new CompraOnline();
    ...
    public List Detalle(int id) {
        List lista = new ArrayList();
        String sql = "select DCW.idDetalleWeb, P.foto, P.nombre, DCW.idComprasweb, DCW.Cantidad, DCW.PrecioCompra FROM detalle_comprasweb DCW inner join producto P on p.idProducto = DCW.idProducto where idComprasweb=" + id;
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                DetalleCompraOnline dcom = new DetalleCompraOnline();
                ...
                //dcom.setIdproducto(rs.getInt(2));
